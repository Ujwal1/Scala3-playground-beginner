  ...
  def main(args: Array[String]): Unit = {
    ...
    class Connection {
      ...
      def get(url: String): String = {
        ...
        else throw new RuntimeException("Can't fetch page rn.")
      }
    ...
    val maybeHTML = maybeConn.flatMap(conn => Try(conn.get(myDesiredURL) )) // map will give details of nested trail of success or failure
    val finalResult = maybeHTML.fold(e => s"<html>${e.getMessage}</html>", s => s) // collapsing the try instance to a successful value or message whan exception happens
    println(maybeHTML)
    println(finalResult)
